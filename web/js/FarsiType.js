...
FarsiType.enable_disable = function(Dis) {
    ...
    if (FarsiType.ShowChangeLangButton === 1) {
        for (let i=1; i<= FarsiType.counter; i++) {
...
FarsiType.Disable = function() {
    FarsiType.Type = false;
    let Dis = document.getElementById('disableFarsiType');
    if (Dis != null) {
        Dis.checked = true;
    }
...
FarsiType.KeyObject = function(z,x) {
    ...
    Convert = function(e) {
        ...
        let eElement = e.target || e.originalTarget || e.srcElement;
        ...
        if (FarsiType.Type) {
            ...
            if (FarsiType.Type && z.farsi) {
                ...
                // to farsi
                try {
                    let docSelection = document.selection;
                    let selectionStart = eElement.selectionStart;
                    let selectionEnd = eElement.selectionEnd;

                    if (typeof selectionStart == 'number') {
                        //FOR W3C STANDARD BROWSERS
                        let nScrollTop = eElement.scrollTop;
                        let nScrollLeft = eElement.scrollLeft;
                        let nScrollWidth = eElement.scrollWidth;
                        ...
                        let nW = eElement.scrollWidth - nScrollWidth;
                        if (eElement.scrollTop === 0) { eElement.scrollTop = nScrollTop }
                    } else if (docSelection) {
                        let nRange = docSelection.createRange();
                        nRange.text = key;
