    ...
    $("#frm-referral").on('beforeSubmit',function(e) {
        ...
        $.ajax(
        {
            ...
            success:function(data) {
                ...
                if(response.ref === 'no')
                {
                    history.pushState(null,'','/officeautomation/reports/inreferralletters');
                    $.pjax.reload({container:'#vw-inrecieveletter-index',async:false});
                    $(".referral_modal_closed").trigger('click');
                    NProgress.done();
                    Swal.fire({
