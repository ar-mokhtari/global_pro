...
/**
 * SendlettersSearch represents the model behind the search form of `app\modules\Codnitive\OfficeAutomation\models\VwRecieveletter`.
 */
class SendlettersSearch extends VwRecieveletter
{
    /**
     * {@inheritdoc}
     */
    ...
    public function search($params)
    {
        ...
        if ($userID) {
            $query = VwRecieveletter::find()->where(['UsersID_FK' => $userID])->
            andWhere(['LettersDraftType' => 1]);
        } else {
            throw new NotFoundHttpException('خـطا: Sendletters/P19');
        }
