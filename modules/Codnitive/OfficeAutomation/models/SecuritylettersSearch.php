...
/**
 * SecuritylettersSearch represents the model behind the search form of `app\modules\Codnitive\OfficeAutomation\models\VwRecieveletter`.
 */
class SecuritylettersSearch extends VwRecieveletter
{
    /**
     * {@inheritdoc}
     */
    ...
    public function search($params)
    {
        ...
        if ($userID) {
            $query = VwRecieveletter::find()->
            where(['UsersID_Reciever' => $userID])->
            orWhere(['UsersID_FK' => $userID])->
            andWhere(['LettersDraftType' => 1])->
            andWhere(['LettersSecurity' => 1]);
        } else {
            throw new NotFoundHttpException('خـطا: SecurityLetter/P15');
        }
