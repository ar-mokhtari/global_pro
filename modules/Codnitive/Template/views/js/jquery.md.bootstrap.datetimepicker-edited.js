...
<div class="popover mds-bootstrap-persian-datetime-picker-popover {{customClass}}" role="tooltip" ${mdDatePickerPopoverFlag}>    
    <div class="arrow"></div>
    <h5 class="popover-header text-center" data-name="mds-datetimepicker-title"></h5>
    <div class="popover-body p-0" data-name="mds-datetimepicker-popoverbody"></div>
</div>
    `;

    var popoverHeaderSelectYearHtmlTemplate = `
<table class="table table-sm table-borderless text-center p-0 m-0 {{rtlCssClass}} {{customClass}}">
    <tr>
        <th>            
...
<div class="mds-bootstrap-persian-datetime-picker-container {{rtlCssClass}} {{customClass}}" ${mdDatePickerContainerFlag}>
...
<td class="border-0" style="{{monthTdStyle}}" {{monthTdAttribute}} data-td-month>
    ...
    function setPopoverHeaderHtml(htmlString, setting) {
        htmlString = `<span class="${setting.customClass}">${htmlString}</span>`;
    ...
    function updateCalendarHtml1($element, setting) {
        ...
        setPopoverHeaderHtml($(calendarHtml).find('[data-selecteddatestring]').text().trim(), setting);
    ...
    function getSelectedDateTimeTextFormatted(setting) {
        ...
        if (setting.rangeSelector && setting.rangeSelectorStartDate != undefined && setting.rangeSelectorEndDate != undefined) {
            if (setting.onlyEndDateRange) {
                return getDateTimeString(!setting.isGregorian ? getDateTimeJsonPersian1(setting.rangeSelectorEndDate) : getDateTimeJson1(setting.rangeSelectorEndDate), setting.textFormat, setting.isGregorian, setting.englishNumber);
            }
    ...
    function getYearsToSelectHtml(setting, yearToStart) {
        ...
        for (var i = yearStart; i < yearEnd; i++) {
            ...
            if (disableBeforeDateTimeJson != undefined && disableBeforeDateTimeJson.year != undefined && currentYearDateTimeNumber < convertToNumber2(disableBeforeDateTimeJson))
                currentYearDisabledAttr = 'disabled';
            if (disableAfterDateTimeJson != undefined && disableAfterDateTimeJson.year != undefined && currentYearDateTimeNumber > convertToNumber2(disableAfterDateTimeJson))
                currentYearDisabledAttr = 'disabled';
            if (setting.disableBeforeToday && currentYearDateTimeNumber < convertToNumber2(todayDateTimeJson))
                currentYearDisabledAttr = 'disabled';
            if (setting.disableAfterToday && currentYearDateTimeNumber > convertToNumber2(todayDateTimeJson))
                currentYearDisabledAttr = 'disabled';
...
`;
    ...
    function getDateTimePickerHtml(setting) {
        ...
        html = html.replace(/{{customClass}}/img, setting.customClass);
        ...
        if (setting.showCalendarTypeToggleBtn) {
            let lang = setting.isGregorian ? 'شمسی' : 'میلادی';
            let calendarTypeToggleBtn = `<a href="javascript:;" class="p-2 calendar-toggle">${lang}</a>`;
            html = html.replace(/{{calendarToggleBtn}}/img, calendarTypeToggleBtn);
        }
        else {
            html = html.replace(/{{calendarToggleBtn}}/img, '');
        }
    ...
    $(document).on('click', mdDatePickerContainerSelector + ' [select-year-button]', function () {
        ...
        popoverHeaderHtml = popoverHeaderHtml.replace(/{{customClass}}/img, setting.customClass);
        ...
        setPopoverHeaderHtml(popoverHeaderHtml, setting);
        ...
        init: function (options) {
            return this.each(function () {
                ...
                if (setting.inLine) {
                    ...
                    popverHtmlTemplate = popverHtmlTemplate.replace(/{{customClass}}/img, setting.customClass);
                    $this.popover({
                        container: 'body',
                        content: '',
                        html: true,
...
$('body').on('click', '.calendar-toggle', function () {
    let popover = $(this).parents('.mds-bootstrap-persian-datetime-picker-popover');
    let popoverId = popover.prop('id');
    let MdPersianDateTimePicker = $(`.datepicker[aria-describedby="${popoverId}"]`);
    let settings = MdPersianDateTimePicker.data('MdPersianDateTimePicker');
    let isGregorian = !settings.isGregorian;
    // MdPersianDateTimePicker.MdPersianDateTimePicker('setOption', 'monthsToShow', isGregorian ? [0] : [0, 1]);
    MdPersianDateTimePicker.MdPersianDateTimePicker('changeType', isGregorian);
    popover.toggleClass('gregorian');
    MdPersianDateTimePicker.MdPersianDateTimePicker('show');
});
